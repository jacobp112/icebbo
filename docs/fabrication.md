...
| `drill-L1-L4.drl` | Plated holes: 128 vias of 0.2 mm, 15 in-pad thermal vias of 0.3 mm, J1's two 1.0 mm pins, and J2's four 0.6 mm shell slots |
| `drill_npth.drl` | J2's two 0.65 mm locating holes |
| `bom.csv` | JLCPCB BOM: Comment, Designator, Footprint and LCSC Part #, with one line per orderable part (34 lines, 91 parts) |
| `pick_and_place.csv` | JLCPCB CPL: designator, centre, layer and rotation for all 91 parts (all on top) |
...
- **Inner planes.** Each plane is drawn as one dark region followed by clear-polarity antipads: 101 on In1 and 122 on In2. These match the regenerated pours one for one. Vias and plated pads are then flashed dark on both inner layers. Non-plane copper is isolated inside its antipad; `check_routing.mjs` requires at least 0.15 mm (see [routing.md](routing.md#checking)). J1's square pin has a square antipad. J2's shell and the in-pad thermal vias join the GND plane and clear the 3.3 V plane by 0.2 mm.
...
- **BOM.** The converter's own BOM put the LCSC number in the value and footprint columns and listed each part on its own line. `fabricate.mjs` writes it from the circuit instead: value and MPN, grouped designators, the footprint (the package names in the script for custom land patterns) and the LCSC number. U4 and U7 have no LCSC number (see [part-selection.md](part-selection.md#ics-crystal-beads-and-connectors)).
...
- **Sourcing.** Source U4 and U7 (not stocked at LCSC) and re-check the low-stock U12 and X2 ([part-selection.md](part-selection.md)).
