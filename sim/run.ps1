...
$rtl = @(
    ...
    (Join-Path $projectRoot "rtl/uart_rx.v"),
    (Join-Path $projectRoot "fpga/timing_probe_top.v")
)
...
foreach ($testName in @("tb_candidate_bbo", "tb_market_data_core", "tb_frame_parser", "tb_vector_replay", "tb_timing_probe")) {
